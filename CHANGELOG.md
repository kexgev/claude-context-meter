...
### Changed
- **Marketplace listing metadata.** Added the AI and Visualization categories
  alongside Other, and expanded the search keywords to the full set of thirty —
  including `fable`, `burn rate`, `token counter`, `session limit` and
  `weekly limit`, so the extension surfaces for the terms people actually search.

No functional changes: the extension code is identical to 2.0.0.
